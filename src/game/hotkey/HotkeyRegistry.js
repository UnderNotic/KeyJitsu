...
export default class HotkeyRegistry {
  ...
  onKeyPressed(pressed) {
    if (
      ...
      (!this.shortcutsToListen || !this.cb)
    ) {
      return;
    }
    ...
    const isShortcutHitCompletely = this.shortcutsToListen.some(s => {
      ...
      const hit = shortcut.every(k => {
        return pressed.map(p => this.keyCodeToHumanReadable(p)).includes(k);
      });
    ...
    const isShortcutHit = this.shortcutsToListen.some(s => {
      const shortcut = s[0];
      return pressed.every(p =>
        shortcut.includes(this.keyCodeToHumanReadable(p))
      );
    });
  ...
  keyCodeToHumanReadable(code) {
    return keycode(code).replace(" ", "");
  }
}
